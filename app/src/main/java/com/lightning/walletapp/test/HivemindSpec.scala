...
import fr.acinq.bitcoin.{Bech32, OP_0, OP_PUSHDATA, OutPoint, Satoshi, Script, Transaction, TxIn, TxOut}
...
class HivemindSpec {
  ...
  def allTests = {
    {
      println("Deposit tx")
      val hivemindDepositHash = ByteVector.fromValidHex("5cb3d8f395c4a43b8add814b1b1bcdd1a050586a")
      ...
      val prevTx = Transaction(version = 0, txIn = TxIn(OutPoint(ByteVector.fromValidHex("00" * 32), 0), ByteVector.empty, 0xffffffffL) :: Nil,
        txOut = TxOut(Satoshi(200000), Hivemind.scriptPubKey) :: TxOut(Satoshi(200000), ByteVector.fromValidHex("00" * 32)) :: Nil, lockTime = 0)
      ...
      val opReturnScript = Script.write(OP_0 :: OP_PUSHDATA(hivemindDepositHash) :: Nil).toArray
      val program = ByteVector.view(ScriptBuilder.createOpReturnScript(opReturnScript).getProgram)
